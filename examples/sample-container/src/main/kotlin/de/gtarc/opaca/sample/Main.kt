...
object SampleLoginHandler: LoginHandler<String>() {
    ...
    override fun get(loginToken: String?) = logins[loginToken]
}
...
fun main() {
    ...
    agentSystem("opaca-sample-container") {
        enable(LocalBroker)
        agents {
            add(ContainerAgent(image, true, SampleLoginHandler))
            add(SampleAgent("sample1", SampleLoginHandler))
            add(SampleAgent("sample2", SampleLoginHandler))
            add(SimpleUIAgent())
            add(SimpleUDPAgent())
        }
    }.start()
