...
A non-required parameter can be omitted entirely from the JSON, or set to `null`, and it is the Agent Container's responsibility to assume a suitable default value. This default value can also be given in the parameter description using the `defaultValue` attribute, but it is not required. (The container may also use a default if the parameter is omitted, and `null` if it is explicitly set to `null`; if ambiguous, this should be described in the action's `description`.)
