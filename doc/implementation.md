...
The REST routes corresponding to the OPACA API are defined in the `PlatformRestController` and preprocessed in the `EventsFilter` for, e.g., creating "events" for certain API calls. The actual implementation logic is distributed over different service classes: `PlatformService`, `ContainersService`, `AgentsService`, and `ConnectionsService`. The main responsibility of the Runtime Platform is twofold: First, it has to manage (start and stop) Agent Containers and connected Runtime Platforms; second, it has to forward incoming requests to the appropriate container or connected platform.

#### Container Management

The logic for managing agent containers is split between a generic part in the `ContainersService` and different backend-specific implementations in the `containerclient` package, e.g., for Docker or Kubernetes. The general procedure is as follows:

* the platform generates a temporary service client for the container (if using auth)
...
When the Agent Container is removed, the backing container is stopped and the respective service client and other information are removed again.

#### Connections Management

When connecting to another Runtime Platform, the `ConnectionsService` will get that platform's info (the containers running there) and add the platform to the list of connected platforms. Note that if the other platform requires [authentication](auth.md), it currently has to use the same Keycloak as the calling platform. The other platform can also be asked to connect back to the calling platform. Connected platforms are notified when containers are added or removed via a websocket connection. Also see [here](protocols.md) for details on the protocol.

#### Message Forwarding

For the routes of the Agent Container API, i.e. `send`, `broadcast` and `invoke`, the `AgentsService` will look up the deployed Agent Container (or connected Runtime Platform) that provides the requested agent or action and forward the call to those. If there are multiple "candidates" that provide the requested action and/or an agent with the given name, then each of those will be tried in turn, starting with locally deployed containers and then connected platforms, until one of the calls succeeds. Only a `broadcast` will always be forwarded to all deployed containers and connected platforms.

Note that in all cases, connected runtime platforms will _only_ be considered if the query-parameter `forward` is `true`. In the forwarded request, the same parameter will be set to `false`, i.e., requests will only be forwarded to directly connected platforms and not over multiple "hops". If the request should only be sent to a specific container, that container's `containerId` can be provided.
...
The Agent Container provides REST routes that allow the outside world to find out about the agents running inside the container and to interact with those agents by sending asynchronous messages (unicast and broadcast) and invoking synchronous actions.
...
* The `sample-container` example does not do anything meaningful, but it uses all the features provided by the API, including, e.g., extra-ports. Its main purpose is to be used in the unit tests for the runtime platform, but it can also be used as a reference for how to implement those features.
...
* The `testing` example includes some cases that may have caused problems in the past, e.g., causing lost messages and similar. It also shows how an agent container can quickly be tested without actually having to deploy it to a Runtime Platform.
