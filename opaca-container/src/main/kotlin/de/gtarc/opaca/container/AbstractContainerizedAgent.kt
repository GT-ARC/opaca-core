...
abstract class AbstractContainerizedAgent(name: String, val description: String? = null): Agent(overrideName=name) {
    ...
    override fun behaviour() = act {
        ...
        respond<StreamPost, Any?> {
            log.info("STREAM POST RESPOND $it")
            when(it.name) {
     ...
     */
    fun <T> sendOutboundInvoke(action: String, agentId: String?, parameters: Map<String, Any?>, type: Class<T>,
                               timeout: Int = -1, containerId: String? = null, forward: Boolean = true): T {
