...
* increment the version numbers in _all_ `pom.xml` files to the next release version, e.g. change `X.Y-SNAPSHOT` to `X.Y`, both the `version` and `parent.version`, and don't forget the examples (there are also a few more places, e.g. in Swagger config or the Dockerfiles; best do a global search for teh current version number)
